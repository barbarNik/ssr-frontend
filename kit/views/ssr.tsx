...
export default class Html extends React.PureComponent<IHtmlProps> {
  public render() {
    const { css, helmet, html, js, styles, environmentVariables, window = {} } = this.props;
    return (
      <html
        lang="en"
      ...
      <script
        dangerouslySetInnerHTML={{
          __html: Object.keys(environmentVariables).reduce(
            (out, key) => out += `window.${key}=${JSON.stringify(environmentVariables[key])};`,
            "",
          ),
        }
        }
